...
import { MapContainer as LeafletMapContainer, TileLayer, Circle, useMapEvents, useMap } from 'react-leaflet';
import { LatLngBounds } from 'leaflet';
import { Shelter, Hotspot, UserLocation, RadiusOption, Language } from '../types';
...
const LocationUpdater: React.FC<{ userLocation: UserLocation | null }> = ({ userLocation }) => {
  const map = useMap();
...
const UserLocationMarker: React.FC<{ location: UserLocation }> = ({ location }) => {
  const map = useMap();
...
export const MapContainer: React.FC<MapContainerProps> = ({
  ...
  const defaultCenter: [number, number] = [25.0330, 121.5654]; // Taipei
  ...
  return (
    ...
    >
      ...
      {visibleShelters.map((shelter) => (
        <ShelterMarker key={shelter.id} shelter={shelter} language={language} />
      ))}

      {visibleHotspots.map((hotspot) => (
        <HotspotMarker key={hotspot.id} hotspot={hotspot} language={language} />
      ))}
    </LeafletMapContainer>
  );
};