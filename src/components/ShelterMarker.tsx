...
export const ShelterMarker: React.FC<ShelterMarkerProps> = ({ shelter, language }) => {
  ...
  return (
    <Marker position={[shelter.lat, shelter.lng]} icon={getShelterIcon(shelter.type)}>
      <Popup className="shelter-popup" maxWidth={300}>
        <div className="p-4 min-w-64">
          <div className="flex items-start justify-between mb-3">
            <h3 className="text-lg font-semibold text-gray-900 pr-2">{shelter.name}</h3>
            <span className={`px-2 py-1 rounded-full text-xs font-medium ${
              shelter.type === 'hospital' ? 'bg-red-100 text-red-700' :
              shelter.type === 'school' ? 'bg-blue-100 text-blue-700' :
              shelter.type === 'community' ? 'bg-green-100 text-green-700' :
          ...
          <div className="space-y-2 mb-4">
            ...
            <div className="flex items-center space-x-2">
              ...
              <a href={`tel:${shelter.phone}`} className="text-sm text-blue-600 hover:text-blue-800">
                {shelter.phone}
              </a>
            </div>
            <div className="flex items-center space-x-2">
              <Users className="h-4 w-4 text-gray-500 flex-shrink-0" />
              <span className="text-sm text-gray-600">{t.capacity}: {shelter.capacity}</span>
            </div>
          </div>
